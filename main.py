...
TOKEN = "8793822580:AAG7HB2ubfo2QrfUPjmi1qnLn1yrXUbxb-s"
ADMIN_ID = 6911800755
...
def vip(m):
    if last_action.get(m.chat.id) == "vip":
        return

    last_action[m.chat.id] = "vip"
...
def add_op(m):
    ...
    bot.send_message(m.chat.id,"Raqam kiriting:")

@bot.message_handler(func=lambda m: m.chat.id in admin_state and "number" not in admin_state[m.chat.id])
def add_num(m):
    op = admin_state[m.chat.id]["operator"]

    cursor.execute("INSERT INTO numbers (operator,number) VALUES (?,?)",(op,m.text))
    conn.commit()
...
def delete_start(m):
    ...
    bot.send_message(m.chat.id, "❌ O‘chirish uchun tanlang:", reply_markup=markup)
...
def delete_number(c):
    ...
    bot.send_message(c.message.chat.id, f"❌ O‘chirildi: {number}")
